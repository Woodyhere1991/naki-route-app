...
function renderRunWeather() {
  ...
  content.innerHTML=(stale?`<p class="weather-alert">${esc(weatherState.error||'Saved forecast — reconnect or refresh before relying on it.')}</p>`:'')+weatherState.towns.map(t=>{
    ...
    return `<section class="weather-town"><h3>${esc(t.name)}${arrive!=null?` <span class="small muted">planned arrival ${esc(fmtClockShort(arrive))}</span>`:''}</h3>${d?weatherDayHtml(d):'<p class="small muted">Forecast not available for the selected date.</p>'}${hours.length?`<details><summary>Forecast through the day</summary><div class="weather-hours">${hours.map(h=>`<div class="weather-day"><b>${esc(h.time.slice(11))}${h.interval_hours>1?' · '+h.interval_hours+'h':''}</b><span>${esc(h.condition)}</span><span>${wxValue(h.temperature)}°</span><span>${wxValue(h.rain_mm)} mm</span><span>Wind ${wxValue(h.wind)} km/h</span><span>Gust ${wxValue(h.gust)} km/h</span></div>`).join('')}</div></details>`:''}<details><summary>Other days</summary><div class="weather-days">${(t.days||[]).map(weatherDayHtml).join('')}</div></details></section>`;
  ...
  content.querySelectorAll('details').forEach((d,i)=>{d.open=Boolean(opened[i]);});
}
